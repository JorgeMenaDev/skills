...
section("slice 3 AI observation and portrayal contracts", () => {
  ...
  check(reference.includes("Optional machine-readable companion experiment") && reference.includes("canonical HTML has already passed") && reference.includes("record `no action`"), "Machine-readable companions must remain conditional on an observed access gap after canonical HTML passes extractability");
  check(reference.includes("one authoritative data source") && reference.includes("Keep canonical HTML primary") && reference.includes("supported HTTP `Link` relation"), "Companion representations must preserve one source of truth and an explicit canonical HTML relationship");
  check(reference.includes("5–25 representative URLs") && reference.includes("Do not blindly add every companion URL to the general sitemap") && reference.includes("duplicate/canonical/index-bloat"), "Companion experiments must start bounded and gate sitemap expansion on canonical/index evidence");
  check(reference.includes("a paid analytics provider is optional and never a dependency") && reference.includes("User-agent text alone is spoofable") && reference.includes("A bot request proves only a request"), "Companion measurement must remain provider-independent, bot-verified where possible, and non-causal");
  check(reference.includes("`llms.txt` as an optional navigation aid, never a ranking signal or required file") && reference.includes("limited read-only API"), "llms.txt and AI-readable APIs must remain optional, bounded companion surfaces");
