...
```bash
node <skill>/scripts/orchestrate-run.mjs dispatch --run <run.json> --expected-revision <n> --slice <id> --executor <id> --session <sid> --conductor-id <id> --conductor-epoch <n>
...
`dispatch` commits `prepared` before `executing`; `observe` binds the real runtime identity and advances to `ACTIVE`. Each command prints its committed revision. Use `validate --run <run.json>` for a read-only contract check. Raw `update --patch` remains the escape hatch.
