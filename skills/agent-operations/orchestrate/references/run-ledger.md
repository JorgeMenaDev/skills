...
Prefer the fenced transition commands for normal progress:

```bash
node <skill>/scripts/orchestrate-run.mjs dispatch --run <run.json> --expected-revision <n> --slice <id> --executor <id> --session <sid> [--vendor <id>] [--model <id>] [--effort <id>] [--runtime-proof <ref>] --conductor-id <id> --conductor-epoch <n>
node <skill>/scripts/orchestrate-run.mjs observe --run <run.json> --expected-revision <n> --slice <id> --session <sid> [--status active|between-calls|complete] --conductor-id <id> --conductor-epoch <n>
node <skill>/scripts/orchestrate-run.mjs handoff --run <run.json> --expected-revision <n> --slice <id> --report <path> --commits <range> --criteria-evidence <json> --conductor-id <id> --conductor-epoch <n>
node <skill>/scripts/orchestrate-run.mjs accept --run <run.json> --expected-revision <n> --slice <id> --conductor-review <ref> (--independent-review <ref> | --waiver <id>) --conductor-id <id> --conductor-epoch <n>
node <skill>/scripts/orchestrate-run.mjs finish --run <run.json> --expected-revision <n> --slice <id> --outcome <outcome> --proof <ref> --conductor-id <id> --conductor-epoch <n>
```

`dispatch` commits `prepared` before `executing`; `observe` binds the real runtime identity and advances to `ACTIVE`. Required vendor/model/effort fields also require `--runtime-proof` naming launcher or runtime metadata evidence; model self-report is never sufficient. Each command prints its committed revision. Use `validate --run <run.json>` for a read-only contract check. Raw `update --patch` remains the escape hatch.
...
`reconcile` compares recorded repo/worktree/process/runtime-observation facts with live state and emits `RECONCILIATION: clean|unknown|offline`. It never guesses clean. `inspect` prints reconciliation age, staleness, every frontier, and a reason for each excluded PLANNED slice; only emitted tokens drive dispatch.
...
When the target branch advances benignly, run `rebase-authority --run <run.json> --expected-revision <n> --to latest [--observations <file>]` (or pin an exact SHA). It accepts only a verified fast-forward whose changed paths do not overlap a nonterminal mutating slice's declared `ownedPaths` or `collisionPaths`, refuses unresolved effects and dirty/advanced PLANNED branches, fast-forwards untouched target-based PLANNED branches, records the decision, invalidates preparations, and re-reconciles. Planned read-only slices hold no write authority, so their path declarations do not block movement; source-bound preparations still invalidate. If `repo.integrationBranch` is declared, `start` binds every slice to its real target or integration head. An integration branch still at the old target is advanced atomically; a diverged integration branch must absorb the target explicitly first. Every integration-based slice's recorded base must remain an ancestor, and changed paths cannot overlap mutating slice authority. Physical slice-branch movement remains PLANNED-only. Raw authority edits are recovery-only.
