...
Material dependencies are the linked artifacts needed to verify a material claim, not every link in a source.

## Claim checks

- Recompute every headline number from its stated inputs. A gain that compares different metrics, windows, or baselines stays marketing until a like-for-like figure exists. Example: "under 5 clicks a week" before and "30,000 impressions a week" after, reported as "47x".
- When the author sells the step they call most important, judge that step's mechanism apart from the product. Adopt the mechanism on its own evidence; the product is a separate `RESEARCH` item.
