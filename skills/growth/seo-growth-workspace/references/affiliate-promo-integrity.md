...
`tracked conversion` → `merchant validation` → `confirmed/approved conversion` → `paid commission` → `net revenue`

A tracked click is not a conversion. A tracked conversion is not confirmed or approved, and none of those states is payout or revenue. Paid commission is gross cash received; net revenue is paid commission minus refunds, cancellations, reversals, chargebacks, invalid leads, program fees, media spend, tooling, and attributable content/operating cost. Record state, amount/currency, period, merchant/program evidence location, checked date, limitations, and adjustments without upgrading one state into another. Code use, direct-code journeys, cookie windows, cross-device paths, QR traffic, last-click rules, and blocked tracking are attribution limitations, not proof of source causation.
