...
For Microsoft surfaces, start from the Bing Webmaster Tools AI Performance export (limits in [data.md](data.md)). Save the CSV under `reports/data/` with its date range. It observes citations only: mentions and recommendations, and every other engine, still need the observation panel below. A run with no saved export records the gap; a missing export is never zero. A grounding query that retrieves a page the site lacks or under-serves is a `gap` or `authority` candidate in the review.
...
Build the initial prompt panel from named, dated demand evidence before the first baseline — never from operator memory or convenience. Acceptable sources: first-party Search Console queries, Bing AI Performance grounding queries (engine search phrases rather than user prompts; record that as their limitation), the site's maintained keyword/query plan, sales-call/support/customer language, live SERP observations for money queries, or explicitly recorded business-context assumptions when first-party evidence does not exist yet. For every prompt preserve, alongside its stable prompt ID, version, and verbatim query: the source and its observation date, the selection rationale, buyer stage, branded/non-branded state, locale/market, intended surface or mode, and the known evidence limitation.
