...
Apply the cooldown, per-rung coverage max-ages, ten-candidate bound, one-Ready-ticket bound, and stage-based SEO Ship cap in `references/operating-policy.md`.

For each observed rung, append its evidence and candidates to the dated sweep ledger, then apply the Binary Eligibility Gate owned by `references/ticket-architecture.md`. Stop at the first eligible candidate, materialize or reuse its ticket through the core contract's fingerprint reconciliation, persist the next-rung cursor, and resolve the run through the three-terminal contract. Retain rejected candidates with their failing gates. P0 evidence may change which candidate wins but never permits a second Ready ticket.

A sweep is complete only when every applicable rung has a coverage artifact within its configured max-age. Completed-sweep and partial-claim rules — including the `nothing valuable this cycle` bar with its top-three rejected candidates — are owned by the terminal contract in `references/never-dry-loop.md`; this reference adds only the traversal mechanics. Always write the dated ledger report from `templates/frontier-sweep-ledger.md`; sweep output is never silence or an unticketed prose conclusion.
