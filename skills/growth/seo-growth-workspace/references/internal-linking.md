...
`siteOrigins[]` is optional and declares the normalized HTTP(S) origin or origin-plus-path prefixes that define internal scope. Prefixes match only on the same origin and at a path boundary. When omitted, the analyzer derives internal scope from the distinct origins in `pages[]`. A target outside that scope is `external`: it remains counted in the edge and anchor inventories, is never called broken, and does not enter the internal graph. Only an internal target absent from `pages[]` is a broken-target finding.
