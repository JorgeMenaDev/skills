...
Before any workspace mutation, the invocation acquires the site lease for the resolved `SITE_WORKSPACE`. Use `scripts/site-lease.mjs`; it records schema 1, `owner`, `runId`, and RFC 3339 UTC `acquiredAt`, `renewedAt`, and `expiresAt`. The sole pre-lease bootstrap exception is ensuring `SITE_WORKSPACE/loops/` exists: the helper performs idempotent `mkdir`, rejects a symlink or non-directory, then immediately competes for the lease by exclusive creation. The empty directory carries no claim; a crash there leaves the workspace safely uninitialized. The protocol is concrete so independent writers interoperate:
...
- **Helper ownership**: acquire with a caller-chosen positive-integer `--ttl-minutes`; renew by matching `owner` + `runId` through same-directory temporary write and atomic rename; release only with that same identity. Renew and release take the recovery lock so they cannot race a stale recovery or each other. An elapsed `expiresAt` is only an `expired_claim`, never proof that the owner is dead. The matching owner may release it; every competing writer remains blocked until the stale-recovery protocol proves liveness failure or a serialized human runbook recovers it.
- **Stale recovery** is serialized by an exclusive recovery lock, never a bare unlink or rename race: a recoverer first acquires `site-lease.recovery.lock` by exclusive creation (create-exclusive/`wx`; the file records recoverer, runId, and time). Holding that lock, it RE-READS the lease and re-probes owner liveness and the bounded stale threshold (a labeled configurable default); only if the lease is still the same stale claim may it unlink the lease and re-acquire by exclusive creation, then release the recovery lock. The lease path stays occupied by the stale file until that unlink, so no fresh acquirer can slip between the re-read and the unlink; a fresh acquirer that wins the exclusive create immediately after the unlink simply beats the recoverer, whose create fails and who backs off to `blocked`. Losers of the recovery lock back off to `blocked`. A recovery lock whose recorded owner fails a liveness probe and whose age exceeds the bounded threshold is never removed automatically - an automatic pathname unlink would recreate the very ABA race this lock serializes. It resolves to honest `blocked`/`needs_human`: a human (or an out-of-band serialized runbook step) removes the orphan, and that manual step is the fencing. Record every recovery.
...
This is cooperative-writer coordination on a trusted local workspace. The helper rejects a pre-existing symlink or non-directory at `loops/`; it is not a security boundary against another process replacing workspace paths during the call.

For a first initialization run: cold-read with `cadence-status.mjs`, acquire the lease, create only evidence-backed schema-1 loop state, release, then cold-read again. The helper creates no cadence, coverage, obligation, ship-event, or certificate data. Seed coverage only from real dated rung artifacts; missing applicable rungs remain missing work, even though the cadence reader can validate only the rows present.
