...
4. **Obligations conformance** — measurement obligations are checked against the current companion contracts, in both directions: every ledger row conforms to the current contracts, and ship history the current contracts would obligate has its companions — a missing companion is filed as a normal backlog row, never fabricated retroactively.
