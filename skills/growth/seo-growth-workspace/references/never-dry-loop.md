...
Deliberate, dated, operator-invoked, one workspace per run; report shape in `templates/upgrade-recap.md`, written to the workspace's `reports/` directory as `reports/YYYY-MM-DD-upgrade-recap-<installed version>.md` (e.g. `reports/2026-07-15-upgrade-recap-5.2.1.md`; the version suffix keeps same-day recaps across consecutive upgrades collision-free). If that path already exists — a same-version rerun, e.g. after a malformed stamp — append the first free numeric suffix (`…-5.2.1-2.md`, `…-5.2.1-3.md`); existing reports are never overwritten. The stamp's `report` field records the path actually written, workspace-relative. Before the checks, record the upgrade context: previous reconciled version (or `never`), installed version, and the load-bearing semantic changes observed between them — this scopes everything below. Under a `never` or `malformed` baseline there is no version pair to compare, and that is the expected shape, not a gap: describe the current protocols' load-bearing demands against the observed workspace state, treating every current protocol as unreconciled (the conservative baseline). One workspace per run bounds each recap, not the fleet: recaps of distinct workspaces are concurrency-safe at the state level — they share no workspace state — but a git working tree is a single shared resource, so concurrent recaps each require their own clone or worktree. In a shared working tree, overlapping recap runs are prohibited outright: the serialization unit is the entire run, from its first workspace read through its push — never just the writes, and never just the git commands — because a sibling's repository-wide rebase or autostash can rewrite state underneath a run that is still reading or writing, leaving conclusions derived from a stale snapshot. Each run stages and commits only its own workspace's paths and pull-rebases before commit and before push, retrying on rejection; the pre-commit pull-rebase runs with autostash over the run's own uncommitted writes — safe precisely because no sibling shares the tree. A recap that changes no backlog row leaves the backlog file untouched entirely, header lines included: the report, log entry, and stamp are the recap's record. Same-day recaps compound: a same-day prior recap's check-1 and check-3 findings satisfy those checks only when **every contract the reused check evaluates** — serialization contracts, rung contracts, the governing policies check 3 tests rows against, and this section's own check-1/check-3 evaluation rules — is byte-identical across the two passes, AND the workspace's loop and ledger state is byte-unchanged since that pass. Prove the state condition (e.g. diff the workspace's `loops/` and ledger paths against the prior recap's commit) and record only the delta; a change to any contract the check evaluates, or to the state, re-runs that check in full. A recap whose four checks all produce zero findings may write the short-form report — header; window evidence in whatever shape the full form would record it (the install-commit pair; the reconstruction evidence or conservative-baseline statement for `never`/`malformed`; per-family pairs where windows differ); the load-bearing semantic-change summary that scopes the checks, or an explicit pointer to an identical same-window summary in a prior same-day report; then per check either its mandatory item-level outcomes in compact one-line-per-item form (per-row keeps, per-rung reasoning — required by checks 2 and 3 regardless of report form) or the zero count where a check reviewed no items; re-stamp — instead of the full template. Only the surrounding narrative collapses, never the item-level evidence or the scoping summary; the full form earns its cost when a check has findings. Four checks, all forward-looking:

1. **Loops-state revalidation** — re-read every schema-1 loop file, ledger, and certificate under the current contracts; malformed or newly non-conforming state is repaired forward or filed, never silently rewritten. Workspace-invented additive fields no contract names (e.g. `capExceptions`, `schedulerMirror`, `lastTerminal`) are tolerated additively: keep them, do not validate them, never delete them; such a field becomes a finding only when it collides with a contract-owned key or meaning. Repairing a stale policy mirror forward (§ Coverage certification) is within this check's repaired-forward authority — a recap counts as "the next write". A finding owned outside the workspace — a hub- or skill-level decision, or a colliding foreign field that cannot be corrected without owning its meaning — takes a fourth disposition, **noted (owned elsewhere)**, naming the owner in the report: not repaired, not deleted, and never filed as site work.
2. **Open-row re-triage** — every open backlog row is re-judged against the current gates; each row gets a dated `keep`, `amend`, or `close` with reason. A `close` moves the row to the backlog's Done/parked table with a closed-by-recap note (closed, not executed) and the report carries the reasoning — the row is never silently deleted. When the closed row carries a dated recheck promise, the close note also names that promise and records its disposition — cancelled with the row (the normal case: the promised action will not run, so nothing is owed) or filed as a normal row where the promise survives the close reason; a promise cancelled this way is a recorded exemption, never an unledgered obligation for check 4's reverse audit. Rows carrying a dated skip disposition are closed for re-triage purposes; skip records living in sweep reports rather than backlog rows are outside re-triage scope entirely. Scope this to the upgrade: when a gate-owning contract is unchanged across the version window, a light conformance scan suffices, and a same-day prior recap's full re-judgment under byte-identical gates satisfies it. Under a `never` baseline no stamp-derived version window exists; the window may be legitimately reconstructed from the consuming repo's installer or version-control history (e.g. the vendored copy's git history proving which skill version actually produced the state), and the light-scan carve-out then applies against that reconstructed window — record the reconstruction evidence in the upgrade context. That source — diffing the installed copy across the consuming repo's install commits — is the canonical window-evidence method for every baseline, not only `never`. State families with different creating versions get their own windows: "the version window" is per state family, not per workspace. When no window can be reconstructed, every open row gets full re-judgment. Label each non-keep outcome `version-driven` or `incidental hygiene`; under a `never` baseline classify by the creating-version test: a finding is `incidental hygiene` when the row violated gates that already existed under the version that produced it, and `version-driven` when it is driven by gates that changed since — or did not exist under — that creating version. When no creating version can be established at all — `never` or unrecoverably `malformed` with no reconstructable window — classify non-keep outcomes `version-driven` by convention and note `creating version unknown` in the reason: the conservative attribution under an unreconciled baseline, since it files the work as normal rows instead of licensing in-pass fixes. Fix findings in-pass only when the amendment is cheap and obvious, otherwise file them as normal rows — the license keys on cost, never on class: a version-driven finding whose amendment is cheap and obvious is equally amendable in-pass, the class records attribution rather than permission, and a filed row whose only content is a one-line amendment of another open row is the wrong exit. The re-triage never becomes the re-audit this pass forbids.
3. **Coverage invalidation review** — coverage-ledger rows certified against rungs or policies that changed (or rungs newly added) are marked stale. Apply the **load-bearing test**: invalidate a rung's row only when the changed clause alters what that rung's method would observe, accept, or reject — its inputs, gates, or evidence semantics; a change outside the rung's remit never invalidates it. Under a `never` baseline "the changed clause" has no referent; the test degenerates to: does the artifact's method, as evidenced, satisfy the current rung contract — invalidate only when it does not. A reconstructed version window (check 2) restores the primary changed-clause test for the rows it covers; the degenerate test applies only where no window exists — re-auditing artifact quality under the degenerate test is the re-audit this pass forbids. A pure policy-mirror change — e.g. a rung's `maxAgeDays` — is never load-bearing by itself and never requires `staleAsOf` method invalidation; the mirror is simply corrected on the next write. Ordinary expiry is a separate question this test does not decide: the reader derives expiry from `observedAt` plus the **current** policy value, so a shortened max-age can make a row expired on its own terms — that is normal staleness surfacing through the coverage reader as candidate work, not recap invalidation, and it never needs the annotation. Record the per-rung reasoning either way. Marking stale means annotating the ledger row with additive `staleAsOf` (date) and `staleReason` fields **and** filing the re-observation as a normal backlog row — the backlog row is the operative exit; the annotation is documentation. The cadence reader derives expiry only from `observedAt + maxAgeDays` and does not honor the annotation.
4. **Obligations conformance** — measurement obligations are checked against the current companion contracts, in both directions: every ledger row conforms to the current contracts, and everything the current contracts would obligate has its ledger entry — ship history's companions, and dated recheck promises recorded in Done rows or log entries (§ Measurement obligations) with neither a ledger entry nor a recorded exemption. A missing companion or unledgered promise is filed as a normal backlog row, never fabricated retroactively. A demand already carried by an existing open row or ledger entry satisfies the exit — record the outcome as `carried by <row or ledger key>` and never file a duplicate; whether a broader row genuinely carries a narrower promise is a judgment, recorded with the outcome. Ships predating the event ledger are adjudicated from evidence at recap time with the report as their dated record — the ship-time fail-closed rule for ambiguous qualification is never applied retroactively.

Then write the dated recap report and re-stamp `reconciledSkillVersion`. History is never rewritten: Done rows, past reports, and ledger history stay exactly as recorded. Within a coverage-ledger row, `observedAt` and `artifact` are that history; the `maxAgeDays` policy mirror is mutable state and the `staleAsOf`/`staleReason` annotations are additive documentation — correcting the mirror and adding those annotations are the only in-place ledger mutations a recap may make, and neither counts as rewriting history. Real work the recap discovers exits into normal new backlog rows — the recap is a bounded bookkeeping pass, never a site re-audit and never a second work queue.
...
- Existing `.seo/loops/<loop-name>.json`: retain current fields and optionally add `schema: 1` or `schemaVersion: 1`, `nextWakeAt`, `wakeOn`, `sleepCertificate`, `occurrences`, `heartbeatAt`, `createdUnderSkillVersion`, and the context-only `stageStamp` (`stage`, evaluation date, evidence basis — canonical key `basis`, governing values being written; an absent basis is never fabricated or renamed into existence — and an optional reason-plus-expiry override). State-creating runs should record `createdUnderSkillVersion` (the running skill version) when they create a loop file, and dated reports name the running skill version in their header — both make later window reconstruction mechanical instead of archaeological. The field is never backfilled: it binds creation-time writers only; a recap that reconstructs provenance records the evidence in its report rather than retrofitting the field. A `stageStamp` basis stored under a non-canonical legacy key is likewise kept as-is — only new writes use `basis`.
...
A dated recheck promise recorded in a Done row or log entry is itself a measurement obligation: it lands in the ledger in the same pass or carries a recorded exemption — a dated note in the recap report, the workspace log, or a Done/parked row's close note, in every case naming the promise and why no ledger entry is owed. Prose next-leads are where obligations go to be forgotten. An inconclusive due measurement records its attempt, reason, and evidence, returns to the same pending lineage with a new `wakeAt`, and is never marked resolved merely because data is late, insufficient, or inaccessible. GSC lag and missing access are inconclusive reasons, not successful outcomes. Deploy verification alone does not resolve a ranking, CTR, conversion, or indexation hypothesis. Apply the measurement timing in `references/operating-policy.md`.
...
For each qualifying or ambiguous SEO Ship, append one normalized event to `.seo/loops/ship-events.json` in the same pass as the ship: a Done row for shipped work either references its ship event or records why the ship does not qualify. Reconcile `dedupeKey` before append so a retry cannot create another event. A multi-URL content batch records one event per counted canonical URL. A qualifying shared release records one event whose `urls` contains its affected URL set.
