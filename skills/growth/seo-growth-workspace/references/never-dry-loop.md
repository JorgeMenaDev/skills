...
- **Stale recovery** is serialized by an atomic rename-claim, never a bare unlink: only when the recorded owner fails a liveness probe AND the lease is past its bounded stale threshold (a labeled configurable default) may a recoverer atomically rename the lease path to a unique recovery-claim path (`site-lease.recovery.<recoverer-runId>.json`). Rename succeeds for exactly one recoverer; a failed rename (the lease was already recovered, released, or replaced) means back off to `blocked` - a recoverer never unlinks or renames a lease it has not just re-read and claimed, so a stale recovery decision can never delete a replacement owner's live lease. The rename winner then re-acquires by exclusive creation (a concurrent fresh acquirer may still win that create; the recovery winner then backs off), unlinks its recovery-claim file, and records the recovery. Orphaned recovery-claim files from crashed recoverers are inert (they are not the lease path) and are cleaned up by the next successful owner.
