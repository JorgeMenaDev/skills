...
Deliberate, dated, operator-invoked, one workspace per run; report shape in `templates/upgrade-recap.md`, written to the workspace's `reports/` directory as `reports/YYYY-MM-DD-upgrade-recap.md` (the stamp's `report` field records that path). Before the checks, record the upgrade context: previous reconciled version (or `never`), installed version, and the load-bearing semantic changes observed between them — this scopes everything below. Four checks, all forward-looking:
