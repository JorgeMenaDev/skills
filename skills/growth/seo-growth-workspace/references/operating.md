...
- **Ship events** (`ship record`, then `loop-state.mjs cap`): every qualifying or ambiguous SEO Ship appends one normalized event in the same pass; the Done row references it or records why the ship does not qualify. `cap` enforces the rolling seven-day stage cap — a cap hit blocks further qualifying publication and routes to `needs_human` or a dated wake; it does not certify sleep and does not manufacture a ticket. Ambiguity fails closed and counts.
