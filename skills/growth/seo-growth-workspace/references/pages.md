...
Use for every new or materially revised public SEO page, whatever the bet's kind. This file owns substantiation (claims, voice, assets, approval) and the launch gates. Evidence vocabulary: the evidence rules in SKILL.md; comparison and disclosure rules: `references/commercial-integrity.md`. A page bet goes `live` only when gates 1–4 have `PASS` evidence; gate 5 needs a logged decision, never a request.
