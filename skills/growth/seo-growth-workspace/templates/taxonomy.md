...
The work-selection order, duplicate rules, empty-backlog rule, and blocker rules live in the skill's `references/operating.md`. Follow that file; do not restate its rules here. An empty Ready/In progress queue does not mean SEO is done — run the smallest useful operating-loop checkpoint and route the checkpoint result through the three-terminal contract in `references/operating.md`.
