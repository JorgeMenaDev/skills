...
- Composite the real logo and one line of copy with `scripts/finish.sh IN OUT.png --size 2560x1440 --logo ... --text ... --font-file <local-brand-font.ttf>` (banner size delegates to `scripts/finish-banner.py`, which shrinks the single line until it fits the safe area and refuses to ship an overflow), then crop x 507, y 508, 1546×423 and look at it: every word and the whole logo sit inside with visible margin.
