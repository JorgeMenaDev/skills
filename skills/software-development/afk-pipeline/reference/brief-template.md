...
- Heading: `### Pipeline` (parser may accept `## Pipeline` too). Keys `verify:` and `recap:`, one per line; the value ends at an optional ` — reason` suffix.
- **Fail-safe:** absent section, unknown key, or unparseable value ⇒ that flag falls back to its default (`verify: full`, `recap: on`, `review: on`). Parsing can only reduce work when the body explicitly and legibly says so.
...
- `review: on` (default) runs an advisory second-model review of the branch diff between implement and verify (vendored `autoreview` skill; codex engine when the runner has it, else claude). Findings trigger a disposition pass that fixes real blockers or rejects with rationale (table committed to the evidence dir). `review: off` for tiny/mechanical diffs where a second model can't beat reading the code.
