...
- `engine: claude` (default) runs the four agent phases on Claude. `engine: codex` runs implement, review-fix, verify, and write-pr on Codex gpt-5.6-luna high with ChatGPT-subscription auth. `engine: cursor` runs implement only on Cursor Agent CLI, then keeps review-fix, verify, and write-pr on Claude because Cursor sessions are non-resumable and structured-output retry needs resume. Cursor v1 is cloud-lane only (`agent:implement`), requires the repo secret `CURSOR_API_KEY`, defaults to model `grok-4.5-xhigh`, and honors a `CURSOR_MODEL` env override. `engine: grok` (v2.11.0 contract; **no deployed runner implements it yet** — current parsers fail-safe it to `engine: claude`) runs implement only on the Grok Build CLI headless (`grok -p`, model `grok-4.5`, effort `high`), then keeps review-fix, verify, and write-pr on Claude; Grok sessions are resumable (`-s`/`-r`), so all-four-phases parity is a permitted later expansion. Grok v1 is local-lane only via the dispatch host's `~/.grok` OAuth (X Premium+; 7-day token, auto-refresh); cloud and sandbox lanes require an `XAI_API_KEY` repo secret or `GROK_AUTH_B64` seed before a grok stamp is honest there. Recap remains Claude. New briefs always stamp `engine:`; if the override is ignored, stamp `engine: claude — override ignored: <reason>` so the durable brief records the decision.
...
- Codex-stamped briefs never overflow to a codex-incapable lane, Cursor-stamped briefs never overflow to docker or sandbox lanes, and Grok-stamped briefs stay on the local lane until cloud/sandbox auth is provisioned. Queue them, or re-cut the brief as `engine: claude — override ignored: <reason>` before labeling.
