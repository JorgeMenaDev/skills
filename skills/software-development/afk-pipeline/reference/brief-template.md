...
recording: on|off — <always stamped; `off` unless the requester asked for recorded proof>
...
- `recording: on` records the primary browser flow and publishes a 30-day Actions artifact plus an inline GIF preview. `agent-browser record start` creates a fresh context, so briefs must put visible URL navigation and scrolling after recording begins. V1 supports the cloud and Vercel Sandbox lanes only; it is incompatible with `verify: off`. Stamp `recording: off` unless the requester explicitly asks for video proof.
