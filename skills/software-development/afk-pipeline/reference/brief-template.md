...
- **Fail-safe:** absent section, unknown key, or unparseable value ⇒ that flag falls back to its default (`verify: full`, `recap: on`). Parsing can only reduce work when the body explicitly and legibly says so.
